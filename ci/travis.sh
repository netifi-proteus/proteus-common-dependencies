...
if [ "$TRAVIS_PULL_REQUEST" != "false" ]; then
    ...
    ./mvnw -s ./settings.xml clean install artifactory:publish -Drevision-prefix=BUILD-SNAPSHOT
elif [ "$TRAVIS_PULL_REQUEST" == "false" ] && [[ "$TRAVIS_BRANCH" == release/* ]] && [ "$TRAVIS_TAG" == "" ]; then
    # Release Branch
    echo -e 'Build Branch for Release => Branch ['$TRAVIS_BRANCH']'
    ./mvnw -s ./settings.xml clean install artifactory:publish -Drevision-prefix=RC
    ...
    ./mvnw -s ./settings.xml clean install artifactory:publish -Drevision-prefix=RELEASE
else
    # Feature Branch
